...
"""
Training script for TrajOptPolicyTF using trajectory optimization data in legged gym.
...
3. Testing the trained transformer policy in the legged gym environment
...
from legged_gym.utils.trajectory_optimization_config import (
...
def get_batch_rollout_task_name(task_name: str) -> str:
    """Convert traj_grad_sampling task name to batch_rollout task name."""
    if "elspider_air_traj_grad_sampling" in task_name:
        return "elspider_air_batch_rollout"
    elif "anymal_c_traj_grad_sampling" in task_name:
        return "anymal_c_batch_rollout"
    elif "go2_traj_grad_sampling" in task_name:
        return "go2_batch_rollout"
    elif "cassie_traj_grad_sampling" in task_name:
        return "cassie_batch_rollout"
    else:
        # Default fallback - assume it's already a batch rollout task
        return task_name


def get_trajectory_config_from_task(task_name: str) -> TrajectoryOptimizationCfg:
    """Get appropriate trajectory optimization config based on task name."""
...
def collect_training_data(
    ...
    save_path: str = "./legged_gym_training_data.pt"
) -> Dict[str, Any]:
    ...
    # Get appropriate batch rollout task name and trajectory config
    batch_rollout_task_name = get_batch_rollout_task_name(task_name)
    trajectory_config = get_trajectory_config_from_task(task_name)
    
    # Override config with provided parameters
    trajectory_config.trajectory_opt.horizon_samples = horizon_samples
    trajectory_config.trajectory_opt.horizon_nodes = horizon_nodes
    
    print(f"Using batch rollout task: {batch_rollout_task_name}")
    print(f"Trajectory config: horizon_samples={trajectory_config.trajectory_opt.horizon_samples}, horizon_nodes={trajectory_config.trajectory_opt.horizon_nodes}")
    
    # Create environment runner v2 for data collection
    runner = LeggedGymEnvRunner2(
        task_name=batch_rollout_task_name,
        num_main_envs=num_envs,
        num_rollout_per_main=128,  # More rollouts for better optimization
        device=device,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(steps_per_episode):
            ...
            traj_sampler.optimize_and_collect_data(
                ...
                obs=obs,
                n_diffuse=1,  # Moderate optimization for speed vs quality
                initial=(step == 0)
            )
            
            # Get next actions and step environment
            actions = runner.get_next_actions()
            obs, priv_obs, rewards, dones, info = runner.env.step(actions)
            ...
            if step % 1 == 0:
                ...
                print(f"  Step {step}: Samples collected: {current_samples}, Avg reward: {rewards.mean().item():.3f}, Time per step: {time_step:.4f}s")
...
def train_transformer_policy(
    dataset_path: str = "./legged_gym_training_data.pt",
    num_epochs: int = 200,
    batch_size: int = 64,
    ...
    checkpoint_dir: str = "./legged_gym_checkpoints"
) -> Dict[str, Any]:
    ...
    # Create environment runner v2 for transformer setup
    batch_rollout_task = get_batch_rollout_task_name("elspider_air_traj_grad_sampling")
    trajectory_config = get_trajectory_config_from_task("elspider_air_traj_grad_sampling")
    trajectory_config.trajectory_opt.horizon_samples = 32
    trajectory_config.trajectory_opt.horizon_nodes = dataset['trajectories_input'].shape[1] - 1
    
    training_runner = LeggedGymEnvRunner2(
        task_name=batch_rollout_task,
...
def test_trained_policy(
    checkpoint_path: str = "./legged_gym_checkpoints/transformer_policy_final.pt",
    ...
    """Test the trained transformer policy in the legged gym environment.
    ...
    # Create environment runner v2 for testing
    batch_rollout_task = get_batch_rollout_task_name(task_name)
    trajectory_config = get_trajectory_config_from_task(task_name)
    trajectory_config.trajectory_opt.horizon_samples = 32
    trajectory_config.trajectory_opt.horizon_nodes = 4
    
    test_runner = LeggedGymEnvRunner2(
        task_name=batch_rollout_task,
        num_main_envs=2 if comparison_mode else 2,
        num_rollout_per_main=1,
        device=device,
        max_steps=steps_per_episode,
        enable_trajectory_optimization=True,
        trajectory_opt_config=trajectory_config,
        headless=not enable_vis
    )
    ...
    trainer = traj_sampler.setup_transformer_training(
        obs_dim=66,  # Will be inferred if observations are used
        **model_config
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train and test TrajOptPolicyTF on legged gym environment")
    ...
    parser.add_argument('--data-steps', type=int, default=200, help='Steps per data collection episode')
    parser.add_argument('--max-samples', type=int, default=100000, help='Maximum samples to collect')
    ...
    parser.add_argument('--test-steps', type=int, default=30000, help='Steps per test episode')
    parser.add_argument('--no-comparison', action='store_true', help='Skip comparison with sampling policy')
    parser.add_argument('--no-vis', action='store_true', help='Disable visualization')
    
    # Environment arguments
    parser.add_argument('--task-name', default='elspider_air_traj_grad_sampling', help='Legged gym task name')
    parser.add_argument('--device', default='cuda:0', help='Device for computation')
    parser.add_argument('--num-envs', type=int, default=10, help='Number of environments')
    
    # File paths
    parser.add_argument('--data-path', default='./legged_gym_training_data.pt', help='Dataset file path')
    parser.add_argument('--checkpoint-dir', default='./legged_gym_checkpoints', help='Checkpoint directory')
    ...
    # NOTE: manually set flags (because argparse conflicts with legged_gym argparse)
    args.collect_data = True
    args.train = False
    args.test = False
    args.all = False  # For testing purposes, run all phases
